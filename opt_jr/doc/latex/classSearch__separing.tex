...
\hyperlink{classSearch__separing}{Search\-\_\-separing} is a policy-\/class\-: it's passed as a template argument to \hyperlink{classSearch}{Search} in order to set the local\-\_\-search method. The local\-\_\-search methods performs first an approximated loop in which the most profitable move is done at each iteration, then it performs an exact loop in which the objective function is evaluated for all possible pairs exchanges at each iteration. 
...
The local\-\_\-search methods performs first an approximated loop in which the most profitable move is done at each iteration, then it performs an exact loop in which the objective function is evaluated for all possible pairs exchanges at each iteration. 
